...
CodeGeneratorMIPSShared::visitMinMaxD(LMinMaxD* ins)
{
    ...
    if (ins->mir()->isMax())
        masm.minMaxDouble(first, second, true, true);
    else
        masm.minMaxDouble(first, second, true, false);
}
...
CodeGeneratorMIPSShared::visitMinMaxF(LMinMaxF* ins)
{
    ...
    if (ins->mir()->isMax())
        masm.minMaxFloat32(first, second, true, true);
    else
        masm.minMaxFloat32(first, second, true, false);
}
