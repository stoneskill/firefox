...
/* LS3 version of S32_Blend_BlitRow32()
...
void S32_Blend_BlitRow32_LS3(SkPMColor* SK_RESTRICT dst,
    ...
    if (count >= 4) {
        ...
        while (count >= 4) {
            ...
            __m128i result = SkPMLerp_LS3(src_pixel, dst_pixel, src_scale);
...
void S32A_Blend_BlitRow32_LS3(SkPMColor* SK_RESTRICT dst,
    ...
    if (count >= 4) {
        ...
        while (count >= 4) {
            ...
            __m128i result = SkBlendARGB32_LS3(src_pixel, dst_pixel, alpha);
            _mm_store_si128(d, result);
            s++;
            d++;
            count -= 4;
...
void Color32A_D565_LS3(uint16_t dst[], SkPMColor src, int count, int x, int y) {
    SkASSERT(count > 0);
...
#define SK_B16x5_B32x5_SHIFT (SK_B32_SHIFT - SK_B16_SHIFT - SK_B16_BITS + 5)
    ...
    #define SkPackedR16x5ToUnmaskedR32x5_LS3(x) (x)
#elif SK_R16x5_R32x5_SHIFT > 0
    #define SkPackedR16x5ToUnmaskedR32x5_LS3(x) (_mm_slli_epi32(x, SK_R16x5_R32x5_SHIFT))
#else
    #define SkPackedR16x5ToUnmaskedR32x5_LS3(x) (_mm_srli_epi32(x, -SK_R16x5_R32x5_SHIFT))
#endif

#if SK_G16x5_G32x5_SHIFT == 0
    #define SkPackedG16x5ToUnmaskedG32x5_LS3(x) (x)
#elif SK_G16x5_G32x5_SHIFT > 0
    #define SkPackedG16x5ToUnmaskedG32x5_LS3(x) (_mm_slli_epi32(x, SK_G16x5_G32x5_SHIFT))
#else
    #define SkPackedG16x5ToUnmaskedG32x5_LS3(x) (_mm_srli_epi32(x, -SK_G16x5_G32x5_SHIFT))
#endif

#if SK_B16x5_B32x5_SHIFT == 0
    #define SkPackedB16x5ToUnmaskedB32x5_LS3(x) (x)
#elif SK_B16x5_B32x5_SHIFT > 0
    #define SkPackedB16x5ToUnmaskedB32x5_LS3(x) (_mm_slli_epi32(x, SK_B16x5_B32x5_SHIFT))
#else
    #define SkPackedB16x5ToUnmaskedB32x5_LS3(x) (_mm_srli_epi32(x, -SK_B16x5_B32x5_SHIFT))
#endif

static __m128i SkBlendLCD16_LS3(__m128i &src, __m128i &dst,
    ...
    __m128i r = _mm_and_si128(SkPackedR16x5ToUnmaskedR32x5_LS3(mask),
                              _mm_set1_epi32(0x1F << SK_R32_SHIFT));

    // g = (0, 0, m0G, 0, 0, 0, m1G, 0, 0, 0, m2G, 0, 0, 0, m3G, 0)
    __m128i g = _mm_and_si128(SkPackedG16x5ToUnmaskedG32x5_LS3(mask),
                              _mm_set1_epi32(0x1F << SK_G32_SHIFT));

    // b = (0, 0, 0, m0B, 0, 0, 0, m1B, 0, 0, 0, m2B, 0, 0, 0, m3B)
    __m128i b = _mm_and_si128(SkPackedB16x5ToUnmaskedB32x5_LS3(mask),
...
static __m128i SkBlendLCD16Opaque_LS3(__m128i &src, __m128i &dst,
                                       __m128i &mask) {
    // In the following comments, the components of src, dst and mask are
    // abbreviated as (s)rc, (d)st, and (m)ask. Color components are marked
    // by an R, G, B, or A suffix. Components of one of the four pixels that
...
void SkBlitLCD16Row_LS3(SkPMColor dst[], const uint16_t mask[],
    ...
    if (width >= 4) {
        ...
        while (width >= 4) {
            ...
            if (pack_cmp != 0xFFFF) {
                ...
                __m128i result = SkBlendLCD16_LS3(src_sse, dst_sse,
                                                   mask_sse, srcA_sse);
                _mm_store_si128(d, result);
            }
...
void SkBlitLCD16OpaqueRow_LS3(SkPMColor dst[], const uint16_t mask[],
    ...
    if (width >= 4) {
        ...
        while (width >= 4) {
            ...
            if (pack_cmp != 0xFFFF) {
                ...
                // Process 4 32bit dst pixels
                __m128i result = SkBlendLCD16Opaque_LS3(src_sse, dst_sse,
                                                         mask_sse);
                _mm_store_si128(d, result);
            }
...
/* LS3 version of S32_D565_Opaque()
...
void S32_D565_Opaque_LS3(uint16_t* SK_RESTRICT dst,
    ...
    if (count >= 8) {
        ...
        while (count >= 8) {
            ...
            __m128i d_pixel = SkPixel32ToPixel16_ToU16_LS3(src_pixel1, src_pixel2);
...
/* LS3 version of S32A_D565_Opaque()
 * portable version is in core/SkBlitRow_D16.cpp
 */
void S32A_D565_Opaque_LS3(uint16_t* SK_RESTRICT dst,
    ...
    if (count >= 8) {
        ...
        while (count >= 8) {
            ...
            dr = _mm_add_epi16(sr, SkMul16ShiftRound_LS3(dr, isa, SK_R16_BITS));
            dr = _mm_srli_epi16(dr, 8 - SK_R16_BITS);
            dg = _mm_add_epi16(sg, SkMul16ShiftRound_LS3(dg, isa, SK_G16_BITS));
            dg = _mm_srli_epi16(dg, 8 - SK_G16_BITS);
            db = _mm_add_epi16(sb, SkMul16ShiftRound_LS3(db, isa, SK_B16_BITS));
            db = _mm_srli_epi16(db, 8 - SK_B16_BITS);

            // Pack R G B into 16-bit color.
            __m128i d_pixel = SkPackRGB16_LS3(dr, dg, db);
...
void S32_D565_Opaque_Dither_LS3(uint16_t* SK_RESTRICT dst,
    ...
    if (count >= 8) {
        ...
        while (count >= 8) {
            ...
            __m128i d_pixel = SkPackRGB16_LS3(sr, sg, sb);
            _mm_store_si128(d++, d_pixel);
...
/* LS3 version of S32A_D565_Opaque_Dither()
 * portable version is in core/SkBlitRow_D16.cpp
 */
...
static const SkBlitRow::ColorProc16 platform_565_colorprocs_LS3[] = {
    Color32A_D565_LS3,                 // Color32A_D565,
    nullptr,                           // Color32A_D565_Dither
};

SkBlitRow::ColorProc16 SkBlitRow::PlatformColorFactory565(unsigned flags) {
#if defined(SK_MIPS_HAS_LS3)
    return platform_565_colorprocs_LS3[flags];
#else
    return nullptr;
#endif
}

static const SkBlitRow::Proc32 platform_32_procs_LS3[] = {
    nullptr,                           // S32_Opaque,
