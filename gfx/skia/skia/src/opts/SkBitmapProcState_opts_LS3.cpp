...
#include "SkBitmapProcState_opts_LS3.h"
...
/*  LS3 version of ClampX_ClampY_filter_scale()
...
void ClampX_ClampY_filter_scale_LS3(const SkBitmapProcState& s, uint32_t xy[],
    ...
    *xy++ = ClampX_ClampY_pack_filter(fy, maxY, s.fFilterOneY);
    ...
    if (dx > 0 && (unsigned)(fx >> 16) <= maxX &&
        ...
        if (count >= 4) {
            // LS3 version of decal_filter_scale
        ...
        // LS3 only support 16bit interger max & min, so only process the case
...
/*  LS3 version of ClampX_ClampY_nofilter_scale()
 *  portable version is in core/SkBitmapProcState_matrix.h
 */
void ClampX_ClampY_nofilter_scale_LS3(const SkBitmapProcState& s,
    ...
    *xy++ = SkClampMax(mapper.intY(), maxY);
    ...
    if ((unsigned)(fx >> 16) <= maxX &&
        (unsigned)((fx + dx * (count - 1)) >> 16) <= maxX) {
        // LS3 version of decal_nofilter_scale
        if (count >= 8) {
            while (((size_t)xy & 0x0F) != 0) {
...
/*  LS3 version of ClampX_ClampY_filter_affine()
 *  portable version is in core/SkBitmapProcState_matrix.h
 */
...
/*  LS3 version of ClampX_ClampY_nofilter_affine()
 *  portable version is in core/SkBitmapProcState_matrix.h
 */
...
void SkBitmapProcState::platformProcs() {
    ...
    if (fSampleProc32 == S32_opaque_D32_filter_DX) {
        fSampleProc32 = S32_opaque_D32_filter_DX_LS3;
    } else if (fSampleProc32 == S32_alpha_D32_filter_DX) {
        fSampleProc32 = S32_alpha_D32_filter_DX_LS3;
    }

    /* Check fMatrixProc */
    if (fMatrixProc == ClampX_ClampY_filter_scale) {
        fMatrixProc = ClampX_ClampY_filter_scale_LS3;
    } else if (fMatrixProc == ClampX_ClampY_nofilter_scale) {
        fMatrixProc = ClampX_ClampY_nofilter_scale_LS3;
    } else if (fMatrixProc == ClampX_ClampY_filter_affine) {
        fMatrixProc = ClampX_ClampY_filter_affine_LS3;
    } else if (fMatrixProc == ClampX_ClampY_nofilter_affine) {
        fMatrixProc = ClampX_ClampY_nofilter_affine_LS3;
    }
}
